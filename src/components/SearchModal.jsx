...
const handleSearch = async () =>{
  ...
  try{
    const normalizedSearchItem = searchItem.toLowerCase();
    const q = query(collection(db, "users"),
    where("username", ">=",normalizedSearchItem),
    where("username", "<=",normalizedSearchItem + "\uf8ff") 
  );
...
console.log(users)
  return (
    <div>
      <button className='bg-[#D9F2ED] w-[35px] h-[35px] p-2 flex items-center justify-center rounded-lg' onClick={openModal}>
        ...
        <div className='relative p-4 w-full max-w-md  max-h-full' onClick={(e) => e.stopPropagation()}>
          < div className='relative bg-[#01AA85] w-[100%] rounded-md shadow-lg'>
            <div className='flex items-center justify-between p-4 md:p-5 border-b border-gray-300'>
              ...
              <button  className='text-white bg-transparent hover:bg-[#d9f2ed] hover:text-[#01AA85] rounded-lg text-sm w-8 h-8  inline-flex justify-center items-center'>
            ...
            <div className='p-4 md:p-5'>
              <div className='space-y-4'>
                <div className='flex gap-2'>
                  <input  onChange={(e)=> setSearchItem(e.target.value)} type="text" className='bg-white border-gray-300 text-gray-900 text-sm rounded-lg outline-none w-full p-2.5' placeholder='Search by name or email'/>
                ...
                {users.map((user) => (
                  <div onClick={() =>{
                    ...
                    closeModal();}
                     ...
                     } className='flex items-start gap-3 bg-[#15eabc34] p-2 rounded-lg cursor-pointer border border-[#ffffff20] shadow-lg mb-3'>
                    <img src={user?.image || defaultAvatar} className='h-[40px] w-[40px] rounded-full' alt="User Avatar" />
                    <span>
